...
.args = if(interactive()) c(getwd(), "model/metavax", "kilifi") else commandArgs(trailingOnly = TRUE)
...
singleRun = function(params, parallel = TRUE){
  ...
  result_aggregated = result_prevacc %>%
    ...
    aggregateModelOutput(model_params,
                         data$epidemiology$prevalence$kilifi_synflorix$data,
...
data_observed = data$epidemiology$prevalence$kilifi_synflorix$data %>%
  .[, N := S + NVT + VT, by="name"] %>%
  .[, age_group := name] %>%
  .[, -"name"] %>%
  melt(measure.vars = c("S", "VT", "NVT"), variable.name="compartment") %>%
  .[, c("mid", "low95", "high95") := binom.confint(value, N, methods="exact")[, c("mean", "lower", "upper")]]

posterior_runs %>%
  dcast(...~compartment, value.var = "value") %>%
  .[, c("VT", "NVT") := .(VT+B/2, NVT+B/2)] %>%
  .[, -"B"] %>%
  melt(measure.vars = c("S", "VT", "NVT"), variable.name = "compartment") %>%
