...
singleRun = function(params, parallel = TRUE){
  ...
  #' then run with routine vaccination
  model_params_current_routine = model_params_current %>% 
    setParameter(level = "vaccination_group", population = c("all"),
                 vaccination_group = "unvaccinated",
                 key = "coverage_r",
                 value = list(
                   list(value = getVaccineCoverage(model_params_current$global_settings$age_groups_model,
                                                   c(set_units(4, "months")), params["routine_coverage"]),
                        time = 0, coverage_to = rep("pcv2", age_groups_model[, .N])))) %>% #send all to pcv2 - assume all get full doses and full VE
    renameCoverageTo2()
  result_routine = runModel(model_params = model_params_current_routine,
                             initial_state = prevacc_equilibrium,
                             steady_state = TRUE,
                             incidence = FALSE)
  result_routine = result_routine$value
  ...
  results_postvacc = lapply(upper_age_limits, function(upper_age_limit){
    ...
    result_postvacc = runModel(model_params = model_params_current,
                               initial_state = result_routine$value,
                               steady_state = FALSE,
                               times = seq(0, 365 * 5, 1),
                               incidence = TRUE)
...
#' iterations per routine scenario
iterations = 500
parameter_run_values = posterior[sample(seq_len(.N), iterations), -"chain"] %>%
  cbind(additional_posteriors$sampler(iterations))

parameter_run_values = lapply(c(0.2, 0.4, 0.6, 0.8), function(routine_cov){
  copy(parameter_run_values) %>%
    .[, routine_coverage := routine_cov]
}) %>% rbindlist()

#' test
#posterior_run = singleRun(unlist(parameter_run_values[1, ]) %>% c(routine_coverage = 0), parallel = FALSE)
...
saveRDS(posterior_runs, sprintf("%s/posterior_runs_routine.RDS", OUTPUT_SUBFOLDER))
saveRDS(parameter_run_values, sprintf("%s/parameter_run_values_routine.RDS", OUTPUT_SUBFOLDER))