...
.args = if(interactive()) c(getwd(), "model/metavax", "kilifi", 2, 1) else commandArgs(trailingOnly = TRUE)
...
ll = function(params, parallel = TRUE){
  ...
  out = calculateLL_kenya(model_params_current, data$epidemiology$prevalence$kilifi_synflorix$data, parallel = parallel)
