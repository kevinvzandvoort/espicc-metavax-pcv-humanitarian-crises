...
posterior_runs = readRDS(sprintf("%s/posterior_runs_main_v2.RDS", OUTPUT_SUBFOLDER))
...
posterior_runs_summarized = posterior_runs %>%
  .[, .(med = median(value),
    ...
    by=c("age_group", "vaccine_strategy", "outcome", "time")]
...
#' Save plotdata for combined plot
saveRDS(posterior_runs_summarized[outcome == "prevalence"],
        sprintf("%s/plotdata_posterior_runs_summarized_prevalence.RDS", OUTPUT_RESULTSFOLDER))
...
ggsave(plot_fig1, filename = sprintf("%s/figure_prevalence_incidence2.png", OUTPUT_RESULTSFOLDER),
       width = PLOT_WIDTH_ONE, height = PLOT_HEIGHT_ONE, units = "in", bg = "#FFFFFF")

posterior_runs_summarized_cum_impact = posterior_runs[outcome == "incidence"] %>%
  .[, .(value_cum = cumsum(value), value = value, time = time), by = c("age_group", "vaccine_strategy", "compartment", "run")] %>%
  dcast(run+time+age_group~vaccine_strategy, value.var = "value_cum") %>%
  melt(measure.vars = vaccination_strategies_plot_settings[dosing == TRUE, model_name],
       variable.name = "vaccine_strategy") %>%
  .[, impact := 1 - value/`<0`] %>%
  .[, .(med = median(impact),
        low50=quantile(impact, 0.25), high50=quantile(impact, 0.75),
        low95=quantile(impact, 0.025), high95=quantile(impact, 0.975)),
    by=c("age_group", "vaccine_strategy", "time")]

posterior_runs_summarized[outcome == "prevalence"] %>%
  saveRDS(sprintf("%s/plotdata_prevalence.RDS", OUTPUT_RESULTSFOLDER))

posterior_runs_summarized_impact %>%
  rbind(data.table(age_group=factor("[0y, 1y)", c("[0y, 120y)", "[0y, 1y)")), vaccine_strategy="<0"), fill=TRUE) %>%
  saveRDS(sprintf("%s/plotdata_incidence.RDS", OUTPUT_RESULTSFOLDER))

posterior_runs_summarized_cum_impact %>%
  rbind(data.table(age_group=factor("[0y, 1y)", c("[0y, 120y)", "[0y, 1y)")), vaccine_strategy="<0"), fill=TRUE) %>%
  saveRDS(sprintf("%s/plotdata_cum_incidence.RDS", OUTPUT_RESULTSFOLDER))
...
saveRDS(incidence_impact, sprintf("%s/results_incidence_impact.RDS", OUTPUT_RESULTSFOLDER))

incidence_impact_summarized = incidence_impact %>%
  .[, .(med = median(value),
      low50=quantile(value, 0.25), high50=quantile(value, 0.75),
      low95=quantile(value, 0.025), high95=quantile(value, 0.975)),
  by=c("age_group", "vaccine_strategy", "variable", "time")]

table_impact = incidence_impact_summarized %>%
  dcast(age_group+vaccine_strategy+time~variable,
  ...
  dcast(age_group+vaccine_strategy~time, value.var = "format") %>%
  merge(vaccination_strategies_plot_settings[, c("model_name", "name")], by.x = "vaccine_strategy", by.y = "model_name") %>%
  .[, vaccine_strategy := name] %>%
  .[, -"name"] %>%
  .[order(-age_group, vaccine_strategy)]
fwrite(table_impact, sprintf("%s/table_impact.csv", OUTPUT_RESULTSFOLDER))
...
table_NNV = incidence_impact_summarized[variable == "NNV"] %>%
  ...
  .[, -"name"] %>%
  .[order(-age_group, vaccine_strategy)]
fwrite(table_NNV, sprintf("%s/table_NNV.csv", OUTPUT_RESULTSFOLDER))

incremental_NNV = incidence_impact[variable == "absolute_impact" & vaccine_strategy == vaccination_strategies_plot_settings[2, model_name],
                                   .(incremental_doses = doses,
                                     incremental_impact = value,
                                     incremental_NNV = doses/value,
                                     comparison = sprintf("%s − %s",
                                                          vaccination_strategies_plot_settings[2, name],
                                                          vaccination_strategies_plot_settings[1, name]),
                                     age_group = age_group, time = time, run = run)] %>%
  rbind(lapply(seq_len(vaccination_strategies_plot_settings[, .N - 2]) + 2, function(i){
    merge(incidence_impact[variable == "absolute_impact" & vaccine_strategy == vaccination_strategies_plot_settings[i, model_name], -"variable"],
          incidence_impact[variable == "absolute_impact" & vaccine_strategy == vaccination_strategies_plot_settings[i - 1, model_name], -"variable"],
          by = c("age_group", "run", "time")) %>%
      .[, .(incremental_doses = doses.x - doses.y,
            incremental_impact = value.x - value.y,
            incremental_NNV = (doses.x - doses.y)/(value.x - value.y),
            comparison = sprintf("%s − %s",
                                 factor(vaccine_strategy.x, vaccination_strategies_plot_settings[, model_name], vaccination_strategies_plot_settings[, name]),
                                 factor(vaccine_strategy.y, vaccination_strategies_plot_settings[, model_name], vaccination_strategies_plot_settings[, name]))),
        by = c("age_group", "time", "run")]
  }) %>% rbindlist())
...
plot_incremental_NNV = incremental_NNV %>%
  ...
  geom_lv(varwidth = TRUE, width.method = "height",
          ...
          colour = lshtm_colours$black,
          fill = lshtm_colours$grey,
  ...
  guides(alpha = "none")+
  theme_minimal()+
  labs(x = "Vaccination strategies compared", y = "Incremental NNV")+
  theme(legend.position = "none", panel.grid.minor.y = element_blank(), strip.text = element_text(face="bold", size=14),
        axis.title = element_text(size=14), axis.text = element_text(size = 12),
        legend.title = element_text(size = 14), axis.text.x = element_text(size = 12, angle = 45, hjust = 1),
        panel.border = element_rect(size=1, fill="#FFFFFF00"))+
  scale_y_log10(labels = function(x) format(x, scientific=FALSE, big.mark=","))+
  coord_flip(ylim=c(10, 1e5))

ggsave(plot_incremental_NNV, filename = sprintf("%s/figure_incremental_NNV.png", OUTPUT_RESULTSFOLDER),
       width = PLOT_WIDTH_ONE, height = PLOT_HEIGHT_ONE, units = "in", bg = "#FFFFFF")

plot_incremental_NNV_3y = incremental_NNV %>%
  .[time == 3] %>%
  ggplot(aes(x = factor(comparison, sprintf("%s − %s", vaccination_strategies_plot_settings[-1, name], vaccination_strategies_plot_settings[-.N, name])),
             xend = factor(comparison, sprintf("%s − %s", vaccination_strategies_plot_settings[-1, name], vaccination_strategies_plot_settings[-.N, name])),
             y = incremental_NNV))+
...
ggsave(plot_incremental_NNV_3y, filename = sprintf("%s/figure_incremental_NNV_3y.png", OUTPUT_RESULTSFOLDER),
       width = PLOT_WIDTH_ONE, height = PLOT_WIDTH_ONETWO, units = "in", bg = "#FFFFFF")
